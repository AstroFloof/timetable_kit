...
class AmtrakGTFSHandler(AgencyGTFSHandler):
    @staticmethod
    def _patch_arizona(stops):
        for index in stops.index:
            if stops.loc[index, "stop_id"] in arizona_stops_list:
                if stops.loc[index, "stop_timezone"] == "America/Denver":
                    ...
                    debug_print(
                        1,
                        "Found Arizona station with wrong timezone: patched"
                        " stop_timezone",
                    )
        return stops

    @staticmethod
    def _patch_buses(feed: FeedEnhanced):
        """
        ...
        for index in routes.index:
            if (
                routes.loc[index, "route_long_name"]
    ...
    @staticmethod
    def _patch_coast_starlight(new_feed: FeedEnhanced):
        """
    ...
    @classmethod
    def patch_feed(cls, feed: FeedEnhanced) -> FeedEnhanced:
        """
        ...
        # The Arizona Problem
        new_stops = cls._patch_arizona(new_feed.stops)
        new_feed.stops = new_stops

        # Buses listed as trains fixes
        cls._patch_buses(new_feed)
